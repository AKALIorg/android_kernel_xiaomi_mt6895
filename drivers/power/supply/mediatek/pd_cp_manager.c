...
static bool pdm_taper_charge(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->tbat > 100 && pdpm->tbat < 150)
		...
		if (pdpm->cycle_count > 400)
			cv_vbat = cv_vbat - 15;
		else if (pdpm->cycle_count > 100)
			cv_vbat = cv_vbat - 10;
		else
			cv_vbat = cv_vbat - 5;
	}

	if (pdpm->isc_alert_level == 3 || pdpm->soa_alert_level || pdpm->bypass_enable)
		cv_vbat = cv_vbat - 5;
#endif
...
#if defined (CONFIG_TARGET_PRODUCT_ARISTOTLE)
	bms_get_property(BMS_PROP_ISC_ALERT_LEVEL, &temp);
	pdpm->isc_alert_level = temp;

	bms_get_property(BMS_PROP_SOA_ALERT_LEVEL, &temp);
	pdpm->soa_alert_level = temp;
#endif
...
static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	...
	else
	{
		...
		fv = pdpm->ffc_enable ? pdpm->dts_config.fv_ffc : pdpm->dts_config.fv  - 5;
#else
